...
const PageOne = (): JSX.Element => {
  ...
  const { throttle: handleContentMouseMove, callback } = useThrottle(
    (e: React.MouseEvent) => {
      const {
        x: left,
        y: top,
      ...
      setStyleParams(
        handleStyleParams(x * 0.1, y * 0.1, x * 0.02, y * 0.1, 0.3, 50),
      );
    },
    100,
  );
  ...
  return (
    <div
      className={classnames("pageOne_container", {
        active: active == steps.one,
      })}
    >
      <div
        className="pageOne_content"
        ref={contentRef}
        onMouseMove={(e) => {
        ...
        style={styleParams}
      >
        <h1 className="pageOne_name__en">Hello, I'm Dual</h1>
        <p className="pageOne_desc__en">
          If you like it, you have to do it to the extreme
        </p>
